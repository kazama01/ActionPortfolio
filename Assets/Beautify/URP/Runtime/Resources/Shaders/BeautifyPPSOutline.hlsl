...
#define BEAUTIFY_PPSOUTLINE
	...
	float OutlinePass(VaryingsOutline i) {
		...
		float outline = 0;
            ...
            float minSat      = max( min(satN, satS), min(satW, satE) );
            float dSat        = maxSat - minSat;
			if (dSat > OUTLINE_MIN_SATURATION_THRESHOLD && dnorm < OUTLINE_EDGE_THRESHOLD && depthDiff > OUTLINE_MIN_DEPTH_THRESHOLD) return 1;
   			return 0;
   		#else
